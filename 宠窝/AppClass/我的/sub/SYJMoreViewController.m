...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *myidea=goodsDic[@"myidea"];
    ...
    ordercell.priceLbl.text=[NSString stringWithFormat:@"¥%@/天",info.price];
    if (myidea.length>0) {
        ordercell.myideaLbl.text=[NSString stringWithFormat:@"我的评价:%@",myidea];
    }
    ordercell.block = ^{
        SYJWriteViewController *writeVC=[[SYJWriteViewController alloc]init];
        writeVC.row=[NSString stringWithFormat:@"%ld",indexPath.row];
        writeVC.block = ^{
            self.goodsAry=[self.store getObjectById:[[NSUserDefaults standardUserDefaults]objectForKey:@"ID"] fromTable:goodstable];
...
//    NSString *roww=goodsDic[@"row"];
//    NSString *sectionn=goodsDic[@"section"];
    NSString *time=goodsDic[@"time"];
    NSString *day=goodsDic[@"day"];
//    SYJmodel *model=self.infoAry[[sectionn longLongValue]];
//    SYJGoodsInfo *info=model.info[[roww longLongValue]];
    UIAlertController *alter=[UIAlertController alertControllerWithTitle:@"信息" message:[NSString stringWithFormat:@"你的预约日期:%@,你的消费天数:%@",time,day] preferredStyle:UIAlertControllerStyleAlert];
